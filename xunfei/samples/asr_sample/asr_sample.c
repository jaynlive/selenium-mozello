...
#define	BUFFER_SIZE 1024 //结果长度	
#define HINTS_SIZE  100	//自定义错误信息长度
#define GRAMID_LEN	128	//语法长度
#define FRAME_LEN	640 //定义帧的大小
...
int get_grammar_id(char* grammar_id, unsigned int id_len)
{
	...
	char buf[80];
 	getcwd(buf,80);
 	strcat(buf,"/xunfei/bin/gm_continuous_digit.abnf");

	fp = fopen(buf, "rb");
	if (NULL == fp)
	{   
		printf("\nopen grammar file failed! %s\n",buf);
		goto grammar_exit;
	}
...
// 获取目录
	char buf[80];
 	getcwd(buf,80);
 	strcat(buf,"/xunfei/bin/wav/word.txt");
 	FILE *pFile;
	...
	if (NULL == f_pcm) 
	{
		...
		pFile = fopen(buf,"w+"); // 文件打开方式 如果原来有内容也会销毁
		//向文件写数据
		fputs("fail",pFile);
		fclose(pFile);
	...
	while (1) 
	{
		unsigned int len = 10 * FRAME_LEN;//2s // 每次写入200ms音频(16k，16bit)：1帧音频20ms，10帧=200ms。16k采样率的16位音频，一帧的大小为640Byte
		int ret = 0;
		...
		if (MSP_EP_AFTER_SPEECH == ep_stat)
			break;
		// usleep(400*1000); //模拟人说话时间间隙，10帧的音频长度为200ms
	}
...
{	
	int			ret						=	MSP_SUCCESS;
	const char* login_params			=	"appid = 563c6c68, work_dir = ."; //登录参数,appid与msc库绑定,请勿随意改动
	/*
	* sub:             请求业务类型
	* result_type:     识别结果格式
	...
	*/
	const char*	session_begin_params	=	"sub=asr,result_type=json,result_encoding=utf8,language=en_us,aue=speex-wb;10";
	...
	// 	goto exit;
	// printf("上传语法成功\n");
	grammar_id = "8d22d6263ec9adfc64cd75f4d1f5b536";

// 获取当前工作路径
  char buf[80];   
  getcwd(buf,80); 
  strcat(buf,"/xunfei/bin/wav/test.wav");
  // printf("current working directory: %s\n", buf); 
 	run_asr(buf, session_begin_params, grammar_id); 
